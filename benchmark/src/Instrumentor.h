...
#define PROFILE 1
	...
	#define PROFILE_BEGIN_SESSION(name, filepath) ::Profiler::Instrumentor::Get().BeginSession(name, filepath)
	#define PROFILE_END_SESSION() ::Profiler::Instrumentor::Get().EndSession()
	#define PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = ::Profiler::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
